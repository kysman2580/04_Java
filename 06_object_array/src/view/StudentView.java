...
//			System.out.println(result); 
	 ...
	 *  -> 추가 성공 시 true, 실패시 false 반환 받아 결과 출력
	 * </pre>
	 */
	private void addStudent() {
		...
		char gender = sc.next().charAt(0); // charAt(0) 0번째 인덱스의 문자를 추출함
	 ...
	 * 
	 * <pre>
	 *  - 인덱스를 입력 받아 StudentService.selectIndex()로 전달
	 *  
	...
	private void selectIndex() {
		...
		// StudentService.selectIndex() 메서드에 전달
		// -> 학생 객체 배열에서 해당 index 번째 학생을 반환 받기
	...
	private void updateIndex() {
		...
		// HTML, CSS, JS, Java 순서로 점수를 입력해라 : 100 20 30 50
		System.out.print("HTML, CSS, JS, Java 순서로 점수를 입력 : ");

		// sc.next() / sc.next000()
		// - 띄어쓰기 / 엔터 입력 전 까지만 인식 ex) 10 20 30 40 -> 10 띄고 20띄고....
		int html = sc.nextInt();
		int css = sc.nextInt();
		int js = sc.nextInt();
		int java = sc.nextInt();
	 ...
	 * 
	 * - 정말 삭제할지 확인 절차 추가 -> "정말 삭제하시겠습니까?(Y/M) : Y"
	 * 
	 * - 'N' 입력 시 "취소 되었습니다" "출력 후 return;
	 * 
	 * - 'Y' 입력 시 1) students배열에서 입력 받은 index 번째 요소를 null로 바꿈
	 * 
...
//		char answer = sc.next().charAt(0); -> char 형태로 변환
//		if(answer == "Y") {}
		...
		// "Y"인 경우
		// 삭제 서비스
		String studentName = service.deleteIndex(index);
