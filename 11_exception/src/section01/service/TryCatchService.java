...
public class TryCatchService {
	 ...
	 * 
	 * Exception Handling(예외 처리) : 발생한 예외를 처리하는 구문
	 * 
	 */
	...
	/**
	 * 예외처리(try-catch) + 다형성(업 캐스팅) + catch 작성 순서 - catch문은 하위 타입 부터 작성 - 서로 상속 관계가
	 * 아닌 Exception에 대한 catch 순서는 관계 없음
	 * 
	 */
	public void test4() {
		...
		try {
			...
			/*
			 * (부모 타입 예외 처리 구문을 먼저 작성하면 (자식 타입) 예외까지 모두 잡기 때문에 뒤에 있는 (자식) catch문이 절대로 실행될 수
			 * 없다
			 **/
		} catch (RuntimeException e) {
	 ...
	 * 
	 * - 중간에 프로그램이 종료되거나 메서드 리턴이 되어도 무조건 실행
	 * 
	 * - 외부 프로그램/장치와의 연결을 끊어낼 떄 주로 사용!
	 * 
	 */
	public void test6() {
		...
		try {
			...
			if(input == 1) System.out.println("예외 없이 정상수행");
			else {
