...
public class BookView {
	...
	public void displayMenu() {
		...
		do {
			...
			case 4:
//				modifyBookPrice();
				modifyBookPrice222222(); // 다른 풀이 방법
				break;
			case 5:
				removeBook();
				break;
	...
	/**
	 * 전체 조회 - BookService의 필드 bookList를 얻어와 일반 for문을 이용하여 모든 요소 정보 출력 단, bookList에
	 * 저장된 정보가 없으면 "저장된 책이 존재하지 않습니다"
	 */
...
//		if(list.size() == 0) {	// 사이즈가 0이면
	...
	/**
	 * 책 정보(제목, 저자, 가격, 출판사)를 입력 받아 BookService의 bookList에 마지막 요소로 추가
	 * 
	...
	/**
	 * 인덱스 번호를 입력 받아 - 해당 인덱스에 책이 없다면 -> "해당 인덱스에 책이 존재하지 않습니다" 출력
	 * 
	 * - 책이 있다면 1) "수정할 가격 입력 : " 출력 2) 스캐너로 가격 입력 받기 3) 입력 받은 index 번째 요소의 가격 수정
	 * "[책제목] 가격이 [이전가격] -> [수정된 가격]으로 수정되었습니다" 출력
	 */
//		private void modifyBookPrice() {
//		System.out.print("\n### 책 가격 수정하기 ###\n");	
//
//			
//		System.out.print("수정 할 책 인덱스 번호 : ");	
//		int index = sc.nextInt();
//		
//		
//		BookDTO book = service.selectIndex(index);
...
//		int price = sc.nextInt();
//		
//		int oldPrice = book.getPrice();	// 이전 가격을 저장
//		book.setPrice(price); // 새 가격으로 변경
//		
//		System.out.printf("[%s] 가격이 [%d] -> [%d]으로 수정되었습니다", 
//				book.getTitle(), oldPrice, price);
//		}

	// 다른 풀이 방법
	private void modifyBookPrice222222() {
		System.out.print("\n### 책 가격 수정하기 ###\n");
		...
		// 서비스 호출
		System.out.println(service.modifyBookPrice222222(index, price));
	}
	...
	private void selectTitle() {
		System.out.println("*** 제목이 일치하는 책 1권 조회 ***");
	...
	private void removeBookTitle() {
		System.out.println("*** 제목이 일치하는 책 제거 ***");

		System.out.print("제거할 책 제목 입력 : ");
		String title = sc.nextLine();

		Boolean target = service.removeBookTitle(title);

		if (target == false) {
			System.out.println("일치하는 책 제목 없음");
		} else {
			System.out.printf("제목이 [%s] 인 책이 제거되었습니다.", title);
		}
	}

	// 출판사가 일치하는 책 모두 조회하기
	private void selectPublisherAll() {
		System.out.println("*** 출판사가 일치하는 책 모두 조회하기 ***");
	...
	private void selectAuthorAll() {
		System.out.println("*** 저자가 일치하는 책 모두 조회하기 ***");

		System.out.printf("저자 입력 : ");
		String author = sc.nextLine();
	...
	private void searchBook() {
		...
		// 서비스 호출
		List<BookDTO> list = service.searchBook(keyword);
