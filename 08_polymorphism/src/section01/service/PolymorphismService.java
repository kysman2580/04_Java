...
//		else if(obj instanceof Object) { 
	...
	/* [바인딩(Binding)]
	 * - 메서드 호출부화 실제 실행할 메서드 코드를 연결하는 것
	 * 
	 * [정적 바인딩]
	 * - 프로그램 실행 전
	...
	public void test3() {
		...
		Parent p1 = new Parent("김", 10);	// 부모 = 부모 참조
		Child c1 = new Child("최", 20, "크스퍼");
		
		Parent p2 = new Child("박", 30, "포터");	// 부모 = 자식 참조(업캐스팅)
		
		// 정적 바인딩 : 프로그램 실행 X 컴파일 단계에서 연결
		
		// (정적) String section01.dto.Parent.toString ()
		System.out.println("p1.toString() : " + p1.toString());
		
		// (정적) String section01.dto.Child.toString ()
		System.out.println("c1.toString() : " + c1.toString());
		
		// (정적) String section01.dto.Parent.toString ()
		// (동적) String section01.dto.Child.toString ()	<- 바인딩이 변경됨
	...
	public void test4() {
		...
		for(Object obj : arr) {	// 향상된 for문
			// 실제 참조 중인 객체의 toString이 호출됨 toString()
	...
	// 다운 캐스팅 시 주의 사항
	// - 강제 형변환이 적용되는 참조 변수가
	// 	 형변환 하려는 객체를 참조하고 있는지 확인 필요
	//-> instanceof 사용	[ 참조변수 instanceof 클래스명 ] 
	//-> 참조변수가 참조하는 객체가 "클래스명" 클래스로 만들어진 객체 또는 상속 받은 객체인 경우 true 반환 아니면 false 반환
	public void test5() {
